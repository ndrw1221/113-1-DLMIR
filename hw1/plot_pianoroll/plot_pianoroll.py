...
def parse_opt():
    ...
    parser.add_argument(
        "--pred_track_dir",
        type=str,
        help="prediction midi track folder path",
        default="../predictions/model_partial-finetune_bs32_ep20_lr1e-4",
    )
    parser.add_argument(
        "--save_dir",
        type=str,
        help="saved fig folder path",
        default="./",
    )
...
def pianoroll_comparison(true_pianoroll, pred_pianoroll, save_path):
    ...
    axes[0].imshow(
        true_pianoroll, aspect="auto", cmap="Oranges", interpolation="nearest"
    )
    ...
    axes[1].imshow(
        pred_pianoroll, aspect="auto", cmap="Oranges", interpolation="nearest"
    )
...
def main(opt):
    ...
    pred_path_list = glob(os.path.join(opt.pred_track_dir, "*.npy"))

    for i, src_path in enumerate(midi_path_list):
        ...
        pred_pianoroll = np.load(pred_path_list[i])
