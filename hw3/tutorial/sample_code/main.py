...
def parse_opt():
    ...
    parser.add_argument(
        ...
        default="../basic_event_dictionary.pkl",
    )
    ...
    with torch.no_grad():
        ...
        if prompt:
            ...
            for _ in range(batch_size):
                ...
                if "chord" in opt.dict_path:
                    tempo_classes = [
                        v for k, v in event2word.items() if "Tempo Class" in k
                    ]
                    tempo_values = [
                        v for k, v in event2word.items() if "Tempo Value" in k
                    ]
        ...
        while current_generated_bar < n_target_bar:
            ...
            word = temperature_sampling(
                logits=_logit, temperature=temperature, topk=topk
            )
...
def train(is_continue=False, checkpoints_path=""):
    ...
    train_list = glob.glob(
        "/home/ndrw1221/nas/datasets/pop1k7/Pop1K7/midi_analyzed/**/*.mid"
    )
    train_list = train_list[0:5]
