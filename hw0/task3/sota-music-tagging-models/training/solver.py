...
class Solver(object):
    def __init__(self, data_loader, config):
        ...
        print("Using device: ", "cuda" if self.is_cuda else "cpu")
    ...
    def get_dataset(self):
        ...
        if self.dataset == "nsynth":
            self.valid_list = np.load("./../split/nsynth/valid.npy")
            self.label = np.load("./data/nsynth/valid/label.npy")
    ...
    def validation(self, best_metric, epoch):
        ...
        # score = 1 - loss
        accuracy, loss = self.get_validation_score(epoch)
        score = accuracy  # Use accuracy as the metric to compare
    ...
    def get_validation_score(self, epoch):
        ...
        for line in tqdm.tqdm(self.valid_list):
            ...
            # ).cuda()
            y = torch.tensor([ground_truth for _ in range(self.batch_size)]).cuda().long()
            out = self.model(x)
            loss = reconst_loss(out, y)
            ...
            # Get predictions
            # Since out is of shape [batch_size, num_classes], get the class with the highest score
            _, predicted = torch.max(out.data, 1)
            est_array.extend(predicted.cpu().numpy())
